...
static bool AppendToFile(const TCHAR *const from, const TCHAR *const dest, const DWORD offset, const DWORD maxlen, unsigned long long *bytesCopied)
{
	*bytesCopied = 0;

	const HANDLE hFrom = CreateFile(from, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
	...
	const HANDLE hDest = CreateFile(dest, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, 0, NULL);
...
NSISFUNC(AppendToFile)
{
	...
	const DWORD maxlen = MAX_VAL(0, popint());
	const DWORD offset = MAX_VAL(0, popint());
	popstringn(dest, 0);
	popstringn(from, 0);
